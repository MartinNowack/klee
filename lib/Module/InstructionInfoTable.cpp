...
InstructionInfoTable::getInfo(const Instruction *inst) const {
  std::unordered_map<const llvm::Instruction *, InstructionInfo>::const_iterator
      it = infos.find(inst);
  if (it == infos.end())
...
InstructionInfoTable::getFunctionInfo(const Function *f) const {
  ...
  std::unordered_map<const llvm::Function *, InstructionInfo>::const_iterator
      it = functionInfos.find(f);
