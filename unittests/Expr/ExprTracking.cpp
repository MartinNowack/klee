...
#include "../../lib/Expr/ConstraintTools.h"
...
TEST(ExprTracking, BasicConstruction) {
  ...
  ref<Expr> read8 = Expr::createTempRead(array, Expr::Int8);

  ref<Expr> neExpr = NeExpr::create(read8, getConstant(5, Expr::Int8));

  // We start with initialized 1, 0 is placeholder for not specified
  ConstraintSetView::next_free_position = 1;

  // Add a simple read
  {
    ExprCountVisitor countingVisitor;
    countingVisitor.visit(neExpr);
    EXPECT_EQ(countingVisitor.getCounter(), 3u);
  }
  cm.addConstraint(neExpr);

  // First added expression: the id is the number of added constraints (width)
  auto pos = constraints.getPositions(constraints.begin());
  EXPECT_EQ(pos.constraint_id, 4u);
  EXPECT_EQ(pos.constraint_width, 3u);

  // Add another expression, which contains the previous expression
  ref<Expr> addExpr =
      AddExpr::create(read8, ConstantExpr::create(8, Expr::Int8));
  {
    ExprCountVisitor countingVisitor;
    countingVisitor.visit(addExpr);
    EXPECT_EQ(countingVisitor.getCounter(), 2u);
  }
  cm.addConstraint(addExpr);
...

  {
    ...
    it2++;
    pos = constraints.getPositions(it2);
    EXPECT_EQ(pos.constraint_id, 6u);
    EXPECT_EQ(pos.constraint_width, 2u);
  }
...

  {
    ...
    it2++;
    pos = constraints.getPositions(it2);
    EXPECT_EQ(pos.constraint_id, 11u);
    EXPECT_EQ(pos.constraint_width, 5u);
  }
  // Check replacement

  ref<Expr> read8_a2 = Expr::createTempRead(array2, 8);
  ref<Expr> eq_expr =
      EqExpr::create(ConstantExpr::create(2, Expr::Int8), read8_a2);
  {}

  llvm::errs() << "Before Replacement\n";

  constraints.dump();
  llvm::errs() << "After Replacement\n";

  cm.addConstraint(eq_expr);
  constraints.dump();
}
